...
public sealed class KeyedServicesExtensionSourceGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // Check if Microsoft.Extensions.DependencyInjection is referenced
        IncrementalValueProvider<bool> referencesDependencyInjection = context.CompilationProvider
            .Select(static (compilation, _) => ReferencesDependencyInjection(compilation.ReferencedAssemblyNames));

        // Combine both conditions
        IncrementalValueProvider<bool> shouldGenerate = isEnabled
            .Combine(referencesDependencyInjection)
        ...
        context.RegisterSourceOutput(shouldGenerate, static (context, shouldGenerate) =>
        {
            if (shouldGenerate)
            {
                context.AddSource("AutoMockerKeyedServicesExtensions.g.cs", KeyedServicesExtensionContent);
            }
        });
    }
    ...
    {
        if (provider.GlobalOptions.TryGetValue("build_property.EnableMoqAutoMockerKeyedServicesGenerator", out var value))
        {
    ...
    private static bool ReferencesDependencyInjection(IEnumerable<AssemblyIdentity> assemblies)
    {
    ...
    private const string KeyedServicesExtensionContent =
        """
        #nullable enable
