...
public sealed class ApplicationInsightsExtensionSourceGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        IncrementalValueProvider<int?> referencesApplicationInsights = context.CompilationProvider
            .Select(static (compilation, _) => GetApplicationInsightsMajorVersion(compilation.ReferencedAssemblyNames));

        // Combine both conditions
        IncrementalValueProvider<int?> shouldGenerate = isEnabled
            .Combine(referencesApplicationInsights)
            .Select(static (tuple, _) => tuple.Left ? tuple.Right : null);

        // Only generate source if enabled and Application Insights is referenced
        context.RegisterSourceOutput(shouldGenerate, static (context, appInsightsMajorVersion) =>
        {
            if (appInsightsMajorVersion <= 2)
            {
